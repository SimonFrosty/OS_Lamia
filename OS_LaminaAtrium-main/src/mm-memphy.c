...
static inline int check_memphy_bounds(struct memphy_struct *mp, addr_t addr) {
    return mp && addr < mp->maxsz;
}
...
{
   if (!check_memphy_bounds(mp, addr) || mp == NULL)
      return -1;
...
int MEMPHY_format(struct memphy_struct *mp, int pagesz)
{
   ...
   fst->fpn = iter;
   fst->fp_next = NULL; /* ensure clean termination when only one frame */
   mp->free_fp_list = fst;
